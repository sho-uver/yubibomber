...
public class SPRT42 : MonoBehaviour
{
    ...
    public void SeparateCall(GameObject gameObject)
    {
        ...
        entry.callback.AddListener((eventDate) => { gameObject.GetComponent<Battle>().Separate42(); });
    ...
    public void VSAI_SeparateCall(GameObject gameObject)
    {
        ...
        EventTrigger.Entry entry = new EventTrigger.Entry();
        entry.eventID = EventTriggerType.PointerDown;
        entry.callback.AddListener((eventDate) => { gameObject.GetComponent<VSAI_Battle>().Separate42(); });
        trigger.triggers.Add(entry);
    }
}